...
export async function requireWorkforceManager(request: NextRequest): Promise<WorkforceContext> {
  ...
  const { data: profile, error: profileError } = await admin.from("profiles")
    .select("company_id,role,status,is_active")
    .eq("id", authData.user.id)
    .single();
  if (profileError || !profile || !["CEO", "PARTNER", "MANAGER", "HR"].includes(String(profile.role)) || profile.status !== "ACTIVE" || !profile.is_active) {
    throw new Error("HR yönetim ekranı yetkisi bulunamadı.");
  }
  return { admin, companyId: String(profile.company_id), userId: authData.user.id };
}
