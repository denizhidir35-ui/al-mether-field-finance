...
export function HrFoundationModule({ user }: { user: AppUser }) {
  ...
  return <div className="grid h-full min-h-0 gap-4 lg:grid-cols-[228px_minmax(0,1fr)]">
    <aside className="mether-surface mether-sidebar mether-scroll grid min-h-0 grid-cols-2 gap-1.5 rounded-[24px] p-3 sm:grid-cols-3 lg:flex lg:flex-col lg:overflow-y-auto">
    ...
    <section className="grid min-h-0 grid-rows-[auto_1fr] gap-4">
      <header className="mether-surface flex items-center justify-between rounded-[24px] px-5 py-4 sm:px-6"><div><div className="text-[9px] font-black uppercase tracking-[.18em] text-blue-400">HR · {selected.label}</div><h1 className="mether-page-title mt-1.5 font-black">{selected.label}</h1></div><button onClick={() => void refresh()} disabled={loading} aria-label="Verileri yenile" className="mether-button-secondary grid h-11 w-11 place-items-center rounded-xl text-slate-500 transition hover:text-blue-600"><RefreshCw size={15} className={loading ? "animate-spin" : ""}/></button></header>
...
function Dashboard({ snapshot }: { snapshot: Snapshot }) {
  ...
  return <div className="grid gap-4"><div className="grid grid-cols-2 gap-3 xl:grid-cols-3">{cards.map(([label, value, Icon]) => <article key={label} className="mether-kpi p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><div><div className="text-[8px] font-bold text-slate-500 sm:text-[9px]">{label}</div><div className="mt-4 text-2xl font-black text-white sm:text-3xl">{value}</div></div><div className="mether-kpi-icon h-9 w-9 shrink-0"><Icon size={16}/></div></div></article>)}</div><article className="mether-surface rounded-[24px] p-5 sm:p-6"><div className="text-[9px] font-black uppercase tracking-[.18em] text-blue-400">Company → Organization → Department → Team → Employee</div><div className="mt-5 grid gap-3 lg:grid-cols-2">{snapshot?.organizations.map(org => <div key={org.id} className="mether-soft-card rounded-2xl p-4 sm:p-5"><div className="text-sm font-black text-white">{org.name}</div><div className="mt-1 text-[9px] text-blue-300">{org.code}</div><div className="mt-4 text-[9px] text-slate-500">{snapshot.departments.filter(dep => dep.organizationId === org.id).length} departman</div></div>)}{!snapshot?.organizations.length ? <Empty text="İlk organizasyonu Organizasyonlar bölümünden oluştur."/> : null}</div></article></div>;
}
...
function FoundationList({ title, count, description }: { title: string; count: number; description: string }) { return <article className="mether-surface rounded-[24px] p-6"><div className="text-[9px] font-black uppercase tracking-[.18em] text-blue-400">HR Foundation</div><div className="mt-3 flex items-end gap-3"><h2 className="mether-page-title font-black">{title}</h2><span className="text-2xl font-black text-blue-500">{count}</span></div><p className="mt-4 max-w-2xl text-[11px] leading-6 text-slate-500">{description}</p><div className="mether-empty-state mt-6 rounded-2xl p-8 text-center text-[10px] text-slate-600">Temel veri modeli, repository ve RLS hazır. Kayıtlar oluştukça bu alan otomatik dolacak.</div></article>; }
function Empty({ text }: { text: string }) { return <div className="mether-empty-state col-span-full rounded-2xl p-7 text-center text-[10px] leading-5 text-slate-500">{text}</div>; }
