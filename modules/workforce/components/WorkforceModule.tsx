...
export function WorkforceModule({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className="grid h-full min-h-0 grid-rows-[auto_1fr] gap-4">
      {!embedded ? <header className="mether-surface rounded-[24px] px-5 py-5 sm:px-6"><div className="text-[9px] font-black uppercase tracking-[.2em] text-blue-400">HR</div><h1 className="mether-page-title mt-1.5 font-black">Çalışanlar</h1><p className="mt-2 text-[10px] text-slate-500">Şefler, yöneticiler ve personel tek çalışan dizininde yönetilir.</p></header> : <div className="sr-only">Çalışanlar</div>}
...
function WorkforceList({ visible, query, sortMode, loading, onQueryChange, onSortChange, onRefresh, onCreate, onImport, onSelect }: {
  ...
  return <section className="mether-surface min-h-0 overflow-hidden rounded-[24px] p-4 sm:p-5">
    ...
    <div className="mether-scroll mt-4 grid max-h-[52vh] gap-2 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-3">
      ...
      {!loading && visible.length === 0 ? <div className="mether-empty-state col-span-full rounded-2xl p-8 text-center"><UsersRound size={22} className="mx-auto text-blue-500" /><div className="mt-4 text-[12px] font-black text-white">{query ? "Aramanızla eşleşen kayıt yok" : "Henüz çalışan bulunmuyor"}</div><p className="mt-2 text-[10px] text-slate-500">{query ? "Arama ifadenizi değiştirerek tekrar deneyin." : "İlk çalışanınızı oluşturarak başlayabilirsiniz."}</p>{!query ? <button type="button" onClick={onCreate} className="mether-button-primary mt-5 h-11 rounded-xl px-5 text-[10px] font-black">Çalışan Ekle</button> : null}</div> : null}
    </div>
  </section>;
}
