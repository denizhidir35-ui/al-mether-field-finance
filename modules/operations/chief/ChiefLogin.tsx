  ...
  return (
    <div className="mx-auto grid h-full w-full max-w-[430px] place-items-center px-3">
      <form onSubmit={submit} className="mether-surface w-full rounded-[28px] border border-blue-400/10 p-5 shadow-[0_28px_80px_rgba(0,0,0,.35)]">
        ...
        <label className="mt-3 block text-[8px] font-bold uppercase tracking-[0.14em] text-slate-500">4 Haneli Şifre<input aria-label="4 Haneli Şifre" type="password" inputMode="numeric" pattern="[0-9]{4}" maxLength={4} value={password} onChange={event => setPassword(event.target.value.replace(/\D/g, "").slice(0, 4))} autoComplete="current-password" className="mt-2 h-12 w-full rounded-2xl border border-white/[0.07] bg-black/20 px-4 text-sm font-bold text-white outline-none transition focus:border-blue-400/35" /></label>
