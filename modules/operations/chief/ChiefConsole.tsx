...
export function ChiefConsole({ onExit }: { onExit?: () => void }) {
  ...
  if (!auth.chief) return <div className="theme-dark-zone fixed inset-0 z-[80] overflow-hidden bg-[#030816] py-3 sm:py-5"><ChiefLogin error={auth.error} onLogin={auth.login} onDevelopmentLogin={auth.loginDevelopment} onExit={onExit ? () => { void auth.logout().finally(onExit); } : undefined} /></div>;
  if (showIntro) return <IntroVideo onFinish={finishIntro} />;
  if (!hydrated) return <div className="theme-dark-zone fixed inset-0 z-[80] grid place-items-center bg-[#030816] p-5"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-blue-300">Operasyon yükleniyor</div></div>;
  ...
  if (assignedWorkOrders.length === 0) return <div className="theme-dark-zone fixed inset-0 z-[80] grid place-items-center bg-[#030816] p-5"><div className="max-w-sm rounded-[28px] border border-blue-300/15 bg-blue-400/[0.04] p-6 text-center"><h2 className="text-lg font-black text-white">Atanmış İş Emri Yok</h2><p className="mt-2 text-[11px] leading-5 text-slate-500">CEO tarafından atanan İş Emirleri burada görünecek.</p><button type="button" onClick={auth.logout} className="mt-5 h-11 rounded-xl bg-blue-600 px-5 text-[10px] font-black text-white">Girişe Dön</button></div></div>;
  return <div className="theme-dark-zone fixed inset-0 z-[80] overflow-hidden bg-[#030816] pb-[env(safe-area-inset-bottom)] pt-[max(.5rem,env(safe-area-inset-top))] sm:py-3"><AuthenticatedChiefConsole chief={auth.chief} workOrders={assignedWorkOrders} projects={readModel.projects} personnelRecords={readModel.personnelRecords} onLogout={auth.logout} /></div>;
}
