...
type TextEntry = { label: string; placeholder: string; value: string; onChange: (value: string) => void };

export function ChiefActionCard({ title, eyebrow, description, actionLabel, complete, disabled, error, fileCapture, textEntry, onAction, onBack }: { title: string; eyebrow: string; description: string; actionLabel: string; complete?: boolean; disabled?: boolean; error?: string | null; fileCapture?: FileCapture; textEntry?: TextEntry; onAction?: () => void | Promise<void>; onBack: () => void }) {
  return (
    <section className="flex min-h-0 flex-1 flex-col rounded-[28px] border border-white/[0.07] bg-white/[0.025] p-4">
      ...
      {textEntry ? <label className="mt-5 block text-[8px] font-black uppercase tracking-[0.13em] text-slate-500">{textEntry.label}<input value={textEntry.value} onChange={event => textEntry.onChange(event.target.value)} placeholder={textEntry.placeholder} className="mt-2 h-12 w-full rounded-2xl border border-white/[0.08] bg-black/20 px-4 text-[12px] font-bold normal-case tracking-normal text-white outline-none focus:border-blue-400/35" /></label> : null}
