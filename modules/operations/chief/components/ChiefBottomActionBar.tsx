import { AlertTriangle, MessageCircle, RadioTower, UsersRound, type LucideIcon } from "lucide-react";
...
export function ChiefBottomActionBar({ activeModule, onSelect }: { activeModule: ChiefModuleId | null; onSelect: (module: ChiefModuleId) => void }) {
  return (
    <nav aria-label="Şef hızlı aksiyonları" className="fixed inset-x-3 bottom-[max(.75rem,env(safe-area-inset-bottom))] z-20 mx-auto grid h-[66px] max-w-[456px] grid-cols-4 rounded-[22px] border border-white/[0.08] bg-[#091224]/95 px-1.5 shadow-[0_20px_55px_rgba(0,0,0,.48)] backdrop-blur-xl">
