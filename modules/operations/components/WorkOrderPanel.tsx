...
export function WorkOrderPanel({ open, defaultProjectCode, onClose }: { open: boolean; defaultProjectCode: ProjectCode; onClose: () => void }) {
  ...
  const [chiefId, setChiefId] = useState(OPERATION_CHIEFS[0].id);
  ...
  return (
    <FocusLayer open={open} onClose={onClose} label="Yeni İş Emri">
      ...
      <form onSubmit={submit} className="mether-scroll grid min-h-0 flex-1 content-start gap-3 overflow-y-auto p-5 md:grid-cols-2">
        ...
        <label className="text-[8px] font-bold uppercase tracking-[0.12em] text-slate-500">Şef<select aria-label="Atanan Şef" value={chiefId} onChange={event => setChiefId(event.target.value)} className={`mt-1.5 ${inputClass}`}>{OPERATION_CHIEFS.filter(chief => chief.status === "active").map(chief => <option key={chief.id} value={chief.id}>{chief.personnelCode} · {chief.displayName}</option>)}</select></label>
        ...
        <fieldset className="rounded-2xl border border-white/[0.06] bg-black/10 p-3 md:col-span-2"><legend className="px-1 text-[8px] font-bold uppercase tracking-[0.12em] text-slate-500">Personel</legend>{readModel.personnelRecords.filter(person => person.status === "active").length === 0 ? <p className="text-[10px] text-amber-300/75">Önce HR ekranından aktif personel oluşturun.</p> : <div className="grid gap-2 sm:grid-cols-2">{readModel.personnelRecords.filter(person => person.status === "active").map(person => <label key={person.id} className="flex items-center gap-2 rounded-xl border border-white/[0.05] p-2 text-[10px] text-slate-300"><input type="checkbox" checked={personnelIds.includes(person.personnelCode)} onChange={() => togglePersonnel(person.personnelCode)} /><span className="font-bold">{person.personnelCode}</span><span className="truncate text-slate-500">{person.displayName}</span></label>)}</div>}</fieldset>
        ...
        <button type="submit" disabled={availableProjects.length === 0 || readModel.personnelRecords.every(person => person.status !== "active")} className="flex h-12 items-center justify-center gap-2 rounded-2xl bg-blue-600 text-[11px] font-black text-white disabled:bg-white/[0.05] disabled:text-slate-600 md:col-span-2"><ClipboardCheck size={17} /> İş Emrini Oluştur</button>
      </form>
    </FocusLayer>
  );
}
